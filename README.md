...
* localへのデータ保存と取り出しの部分の仕組みがよく理解できてなかった。
* clear `\n`は改行
* 関数の引数の仕組みがよく分かってなかった。ついでにイベントハンドラの呼び出しの仕組みもよく分かってない、戻って勉強し直す。
...
## タスク管理アプリの仕様
* 「今日のタスク」と「タスクの達成状況」を記載できる。
* 前の月のタスクも確認可能。
* タスクを書いた日は黄色の太文字にすることで達成感を感じられるようにしてる。
...
* clear 文字サイズをあげる
* clear 諸々のコメントを消す
* cleat ログは別で残しつつ、このREADMEには作り方や機能の説明をする
* clear 各メソッドなどを別の書き方で書き換えてみる